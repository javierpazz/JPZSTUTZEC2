...
export default function CtaCusListScreen() {
  const [
    {
      loading,
      error,
    ...
    const fetchData = async () => {
      try {
        ...
        // const { data } = await axios.get(`${API}/api/receipts/searchingegrSB?fech1=${fech1}&fech2=${fech2}&configuracion=${codCon}&usuario=${codUse}&encargado=${codEnc}`,{
           const { data } = await axios.get(`${API}/api/invoices/ctacus/?configuracion=${codCon}&order=${order}&fech1=${fech1}&fech2=${fech2}&usuario=${codUse}&customer=${codCus}`, {
                headers: { Authorization: `Bearer ${userInfo.token}` },
      });
  ...
  const parametros = async () => {
    navigate('/admin/filtros?redirect=/admin/informe/CtaCus');
  };
        ...
        {(isDet) ? (
                <div className="p-4 space-y-10">
                  <h1 className="text-2xl font-bold mb-6">
                    Cuenta Corriente Clientes - Total General: ${saldoTotalGeneral.toFixed(2)}
                  </h1>
        
                  {cuentas.map((cuenta) => (
                    <div key={cuenta.id_client} className="border rounded-xl p-4 shadow-md">
                      <h2 className="text-xl font-semibold text-blue-700 mb-4">
                     { `${cuenta.codCust} - ${cuenta.nombreCliente}`}
                      </h2>
                ...
                {cuentas.map((cuenta) => (
                  <div key={cuenta.id_client} className="border rounded-xl p-4 shadow-md">
                    <table className="w-full text-left border-collapse">
                      ...
                      <tfoot>
                        <tr className="bg-gray-100 font-bold">
                          <td className="p-2 border" colSpan={7}>{ `${cuenta.codCust} - ${cuenta.nombreCliente}`}</td>
                          <td className="p-2 border text-end">${cuenta.saldoTotal.toFixed(2)}</td>
                        </tr>
                      </tfoot>
                    </table>
